...
def main():
    ...
    if upload_file is not None:
        ...
        if fitur:
            ...
            st.write(f"Jumlah Data Pelatihan: {len(x_train)}")
            st.write(f"Jumlah Data Pengujian: {len(x_test)}")

            # Visualisasi perbandingan
            st.write("Distribusi Data Pelatihan dan Pengujian")
            fig, ax = plt.subplots()
            ax.bar(['Training Data', 'Testing Data'], [len(x_train), len(x_test)], color=['blue', 'orange'])
